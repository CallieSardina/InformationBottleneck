...
def save_activations(activations, epoch, batch_idx, layer_dir='layer_activations_mi'):
...
def train(model, optimizer, train_loader, epoch, device):
    ...
    for batch_idx, train_batch in tqdm(enumerate(train_loader), desc='Train Batch', total=len(train_loader)):
        ...
        with open('./run_results.txt', 'a') as f:
            print(f"Epoch {epoch}, Batch {batch_idx}, MI_XZ: {mi_XZ_1}, {mi_XZ_2}, {mi_XZ_3}, MI_ZY: {mi_ZY_1}, {mi_ZY_2}, {mi_ZY_3}", file=f)
...
def main():
    ...
    # Logging.
    gnn_folder = f'data/{args.dataset}/gnn_mi/'
