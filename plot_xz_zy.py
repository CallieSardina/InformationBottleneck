...
with open(log_file, 'r') as file:
    for line in file:
        ...
        if match:
            ...
            mi_zy1 = float(match.group(6))
            mi_zy2 = float(match.group(7))
            mi_zy3 = float(match.group(8))

            # Append the extracted values to the lists
            mi_xz_values.append((mi_xz1, mi_xz2, mi_xz3))
            mi_zy_values.append((mi_zy1, mi_zy2, mi_zy3))
            zi_values.append([1, 2, 3])  # Z_i is just the layer number (1, 2, 3) for each entry
            epochs.append(epoch)
